...
var treatList = {
    ...
    initTable: function () {
        this.dataTable = $('#myTable').dataTable({
            sAjaxSource: base+'/treat/list',   //ajax地址
            ...
            aoColumns: [                //列定义
                {
                    ...
                    mRender: function (data, type, full) { //渲染函数,即 td 内显示的内容
                        return '<a href="javascript: treatDetail(' + full.id + ')">' + full.telephone + '</a>';
                    }
                ...
                },
                {
                    ...
                    mRender: function (data, type, full) {
                        ...
                        var html = '';
                        if(status == 2){
                            html += '<button class="btn btn-outline btn-danger" onClick="toTreatModel(' + full.id + ')">实名认证</button>';
                        }
                        if(status == 1 || status == 2 || status == 3){
                            html += '<button class="btn btn-outline btn-info" onClick="treatDetail(' + full.id + ')">详情</button>';
                        }
